...
operators.forEach((operator) => {
  operator.addEventListener("click", (e) => {
    ...
    let dis: string = display.textContent!;
    if (dis !== "") {
      const splitval = dis.split("");
      const length: number = splitval.length - 1;
      let lastVal: string | number = splitval[length]; //Get the last value
      if (lastVal == "+" || lastVal == "-" || lastVal == "/") {//check if it is function
        const slicedDis: string | number = dis.slice(0, length); //slice the string to get the string without the last value
